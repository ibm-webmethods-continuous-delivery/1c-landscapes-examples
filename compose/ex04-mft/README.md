...
This example uses a Postgres database service with two separate databases: one for the product and one for archiving. The database creation image is the one provided officially with MFT by means of the container image `ibmwebmethods.azurecr.io/webmethods-activetransfer-dcc:11.1`.
